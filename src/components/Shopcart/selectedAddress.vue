<template>
  <div class="select_address" ref="myAddress">
    <div class="container">
      <div class="header">
          ...
          <div class="address_box">
            <div class="address_type">
              ...
              <div class="goodsListItemStatus" @click="selectOne(item)">
                <svg-icon v-if="currentIndex == item.id" icon-class="selected" style="width: 18px;height: 18px;background: #ff6a20;border-radius: 50%;"></svg-icon>
...
export default {
  ...
  methods: {
    ...
    goBack () {
      this.$router.push('/shopcart/settlement?phone=' + this.listQuery.account + '&selected=' + this.currentIndex)
    },
    onEdit (row) {
    ...
    selectOne (row) {
      ...
      // this.$root.eventHub.$emit('selectAddress', this.currentIndex)
    },
    handleCancel () {
      this.showDialog = false
    },
    ...
    // offEventHubListener () {
    //   this.$root.eventHub.$off('selectAddress')
    // }
  },
  // beforeDestroy () {
  //   // 离开页面之前，解除事件绑定
